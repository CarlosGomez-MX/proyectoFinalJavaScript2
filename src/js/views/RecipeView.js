  ...
  _generateMarkup() {
    ...
    return `
      ...
      <div class="recipe__details">
        <div class="recipe__info"><span>⏱ ${this._data.cookingTime} min</span><span>👥 ${this._data.servings} porciones</span></div>
        <div class="recipe__publisher">${this._data.publisher ?? ''}</div>
      </div>
      <div class="recipe__ingredients">
        <h2>Ingredientes</h2>
        <ul class="recipe__ingredient-list">
          ${(this._data.ingredients ?? []).map(i => `<li>${i.quantity ?? ''} ${i.unit ?? ''} ${i.description ?? ''}</li>`).join('')}
        </ul>
      </div>
